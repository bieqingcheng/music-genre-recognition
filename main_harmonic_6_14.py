...
from model_harmonic_6_14 import model_audio
from dataloader_harmonic_6_14 import datatype
...
def main():
    ...
    for epoch in range(0, epoches-start_epoch):
        ...
        with open(Config["harmonic_6_14"]["harmonic_train_process_txt"].format(ff), 'a') as f:
            f.write(line)
        ...
        if is_best:
            best_acc = test_acc
            dir_best = Config["harmonic_6_14"]["harmonic_model_best"].format(ff)
            print(dir_best)
...
def test(val_loader, model, criterion, criterion_cent, epoch, use_cuda):
    ...
    out_w = Config["harmonic_6_14"]["save_result"].format(mode,ff)
    feat_w = Config["harmonic_6_14"]["extract_feature"].format(mode,ff)
