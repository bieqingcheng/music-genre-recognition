...
from model_fusion_sT_6_14 import model_audio
from dataloader_sT_fusion import datatype
...
def main():
    ...
    opt = args
    args.lr = 1e-5#Config["normal_config"]["initial_lr"]
    ...
    if args.resume: 
        ...
        checkpoint = torch.load(Config["st_6_14"]["st_checkpoint"].format(ff))
    ...
    for epoch in range(0, epoches-start_epoch):
        ...
        with open(Config["st_6_14"]["st_train_process_txt"].format(ff), 'a') as f:
            f.write(line)
        dir = Config["st_6_14"]["st_checkpoint"].format(ff)
        ...
        if is_best:
            ...
            dir_best = Config["st_6_14"]["st_model_best"].format(ff)
            print(dir_best)
...
def train(train_loader, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, ss  in enumerate(train_loader):
        ...
        #import pudb;pu.db
        #melspectrogram  = ss[3].unsqueeze(1)
        melspectrogram  = ss[1]
        path = ss[0]
        ...
        #with open(Config["melspect_config"]["train_epoch_melspectrogram_4"], 'a') as f:
        #    f.write(line)
        if batch_idx%50==0:
...
def test(val_loader, model, criterion, epoch, use_cuda):
    ...
    for batch_idx, ss in enumerate(val_loader):
        ...
        prec1= accuracy(outputs.data, targets.data, topk=(1,))
        #import pudb;pu.db
        #prec1a, size1 = accuracy1(outputs.data, targets.data, path1)
        losses.update(loss.item(), inputs.size(0))
        top1.update(prec1[0].item(), inputs.size(0))
        #top1.update(prec1a, size1)
        #print('test:{}/top1:{}/loss:{}'.format(batch_idx,acc1,losses.avg))
        if batch_idx%10==0:
    ...
    out_w = Config["st_6_14"]["save_result"].format(mode,ff)
    feat_w = Config["st_6_14"]["extract_feature"].format(mode,ff)
