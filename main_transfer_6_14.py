...
from model_transfer_6_14 import model_audio

from dataloader_transfer_6_14 import datatype
...
def main():
    ...
    if args.resume:
        ...
        checkpoint = torch.load(Config["transfer_6_14"]["transfer_model_best"].format(ff))
    ...
    for epoch in range(0, epoches-start_epoch):
        ...
        with open(Config["transfer_6_14"]["transfer_train_process_txt"].format(ff), 'a') as f:
            f.write(line)
        dir = Config["transfer_6_14"]["transfer_checkpoint"].format(ff)
        ...
        if is_best:
            ...
            dir_best = Config["transfer_6_14"]["transfer_model_best"].format(ff)
            print(dir_best)
...
def test(val_loader, model, criterion, criterion_cent, epoch, use_cuda):
    ...
    out_w = Config["transfer_6_14"]["save_result"].format(mode,ff)
    feat_w = Config["transfer_6_14"]["extract_feature"].format(mode,ff)
