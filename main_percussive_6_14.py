...
from model_percussive_6_14 import model_audio
from dataloader_percussive_6_14 import datatype
...
def main():
    ...
    if args.resume:
        checkpoint = torch.load(Config["percussive_6_14"]["percussive_model_best"].format(ff))
    ...
    for epoch in range(0, epoches-start_epoch):
        ...
        with open(Config["percussive_6_14"]["percussive_train_process_txt"].format(ff), 'a') as f:
            f.write(line)
        dir = Config["percussive_6_14"]["percussive_checkpoint"].format(ff)
        ...
        if is_best:
            best_acc = test_acc
            dir_best = Config["percussive_6_14"]["percussive_model_best"].format(ff)
            print(dir_best)
...
def test(val_loader, model, criterion, criterion_cent, epoch, use_cuda):
    ...
    out_w = Config["percuusive_6_14"]["save_result"].format(mode,ff)
    feat_w = Config["percussive_6_14"]["extract_feature"].format(mode,ff)
